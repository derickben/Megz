...
router.get('/handle/:handle', (req, res) => {
  ...
  Profile.findOne({handle: req.params.handle})
    .populate('user', ['name', 'avatar']) // name and avatar is specified to prevent the user from adding other fields such as email and password 
    .then(foundProfile => {
      if (!foundProfile) {
        errors.noprofile = 'There is no profile for this user';
  ...
  if (!isValid) {
   ...
   Profile.findOne({user: req.user.id}).then(foundProfile => {
      if (foundProfile) {
        ...
        Profile.findOne({handle: req.user.handle}).then(foundProfile => {
          if (foundProfile) {
  ...
  Profile.findOne({user: req.user.id})
    .then(foundProfile => {
      ...
      const removeIndex = foundProfile.experience
        .map(item => item.id)  // this map method returns elements with id, you could say item.name to return elements with name
      ...
      // Save
      foundProfile.save().then(foundProfile => res.json(foundProfile));
    })
    .catch(err => res.status(404).json(err));
});
